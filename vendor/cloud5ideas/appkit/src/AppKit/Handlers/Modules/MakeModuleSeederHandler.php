...
class MakeModuleSeederHandler
{
	...
	public function fire(Command $console, $slug, $class)
	{
		...
		if ($this->module->exists($this->moduleName)) {
			...
			return $this->console->info("Created Module Database Seeder: [$this->moduleName] ".$this->getFilename());
		}
	...
	{
		return studly_case($this->className).'TableSeeder.php';
	}
	...
	{
		return $this->formatContent($this->finder->get(__DIR__.'/../../Stubs/Modules/ModuleTableSeeder.stub'));
	}
	...
	{
		return str_replace(
			['{{className}}', '{{moduleName}}', '{{namespace}}', '{{pluralClassName}}', '{{singularClassName}}'],
			[$this->className, $this->moduleName, $this->module->getNamespace(), str_plural(strtolower($this->className)), str_singular(strtolower($this->className))],
			$content
		);
	}
}
