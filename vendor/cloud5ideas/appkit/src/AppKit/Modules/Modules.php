<?php namespace C5\AppKit\Modules;
...
class Modules implements Countable
{
	...
	{
		return $this->path ?: $this->config->get('appkit.modules_path');
	}
	...
	{
		return $this->config->get('appkit.modules_namespace');
	}
	...
	/**
	 * Get all modules that are navigable.
	 *
	 * @param  bool $enabled
	 * @return array
	 */
	public function getByNavigable($enabled = true)
	{
		$disabledModules = array();
		$enabledModules  = array();
		$modules         = $this->all();
		...
		foreach ($modules as $module) {
			...
			if($this->isNavigable($module['slug'])){
				if ($this->isEnabled($module['slug'])) {
					$enabledModules[] = $module;
					$order[]          = $module['order'];
				} else {
	...
	/**
	 * Simple alias for getByNavigable($enabled).
	 *
	 * @return array
	 */
	public function navigable($enabled = true)
	{
		return $this->getByNavigable($enabled);
	}
	...
	/**
	 * Check if specified module is navigable.
	 *
	 * @param  string $slug
	 * @return bool
	 */
	public function isNavigable($slug)
	{
		return $this->getProperty("{$slug}::navigable") === true;
	}
